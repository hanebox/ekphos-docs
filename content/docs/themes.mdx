...
Official themes bundled with ekphos are listed first and tagged `official`; any custom themes you've added to `~/.config/ekphos/themes/` follow, tagged `custom`. Confirming with `Enter` writes the choice to `theme` in your [config](/docs/configuration), so it persists across restarts.
